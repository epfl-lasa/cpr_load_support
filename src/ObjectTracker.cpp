...
ObjectTracker::ObjectTracker(
	...
	ROS_INFO_STREAM("Calibration: make sure markers are visibles: "
	                << frame_mocap_calibration_ << " and "
	                << frame_mocap_endeffector_);

	ROS_WARN_STREAM("Make sure the z-axis for both"
	...
	// while (nh_.ok()) {
	// 		transform_robot_ee_calib.stamp_ = ros::Time::now() + time_diff_;
	// 		transform_robot_ee_calib.frame_id_ = frame_robot_endeffector_;
	// 		transform_robot_ee_calib.child_frame_id_ = "fake";
	// 		brodcaster_.sendTransform(transform_robot_ee_calib);
	// 	ros::spinOnce();
	// 	loop_rate_.sleep();
	// }
	...
	ROS_INFO_STREAM("in robot: " << origin1.getX() << " " << origin1.getY() << " " << origin1.getZ() );
	ROS_INFO_STREAM("in mocap: " << origin2.getX() << " " << origin2.getY() << " " << origin2.getZ() );

	// ros::Publisher pub_target_ = nh_.advertise<geometry_msgs::PointStamped>("my_point", 1);

	// geometry_msgs::PointStamped msg;

	// msg.header.frame_id = frame_mocap_endeffector_;
	// msg.point.x = origin2.getX();
	// msg.point.y = origin2.getY();
	// msg.point.z = origin2.getZ();
...
// assuming the two frames matching in z-axis
	...
	ROS_INFO_STREAM("Calibraton is finished. rotation: " << w
	                << " around " << xyz.getX() << " " << xyz.getY() << " " << xyz.getZ() );
	...
	ROS_INFO_STREAM("Calibraton is finished. Quaternion offset: " << quat_offset_.getAngleShortestPath() << "[rad]");
...
void ObjectTracker::Run() {
	...
	while (nh_.ok()) {
		...
		try {
			...
			transform.frame_id_ = frame_robot_endeffector_;
